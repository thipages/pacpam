...
import { GUARD_TRANSITIONS, byKey, GUARD_STATE_ORDER } from './guard-transition-ids.js';
...
async function guardCoverage(driver, log) {
  log('step', 'Validation du registre');
  validateRegistry();
  log('pass', `${GUARD_TRANSITIONS.length} IDs correspondent à guardStates`);

  for (const state of GUARD_STATE_ORDER) {
    const config = guardStates[state];
    ...
    for (const [event, transition] of events) {
      ...
      const sm = new StateRunner(guardStates, guardInitial);
      sm.current = state;
  ...
  log('step', 'Bilan');
  log('pass', `${GUARD_TRANSITIONS.length} transitions = 100%`);
}

function validateRegistry() {
  for (const t of GUARD_TRANSITIONS) {
    const stateConfig = guardStates[t.from];
    if (!stateConfig?.on) throw new Error(`#${t.id}: état ${t.from} inconnu dans guardStates`);
  ...
  for (const [state, config] of Object.entries(guardStates)) {
    if (!config.on) continue;
    for (const event of Object.keys(config.on)) {
      if (!byKey.has(`${state}:${event}`)) {
        throw new Error(`${state}:${event} absent du registre guard-transition-ids.js`);
      }
    }
  }
}
...
function renderTimeline(container) {
  ...
  legend.innerHTML = `<span class="sm-legend-item peerjs">✓ ${GUARD_TRANSITIONS.length} transitions testées</span>`;
  container.appendChild(legend);

  const el = document.createElement('div');
  el.className = 'sm-timeline';

  for (let i = 0; i < GUARD_STATE_ORDER.length; i++) {
    const state = GUARD_STATE_ORDER[i];
    ...
    if (i < GUARD_STATE_ORDER.length - 1) {
